...
void WireEepromWriteOneByte(int theDeviceAddress, unsigned int theMemoryAddress, byte theByte) 
{
	...
	Wire.beginTransmission(theDeviceAddress);
	Wire.send((byte)((theMemoryAddress ) >> 8));
	Wire.send((byte)((theMemoryAddress) >> 0));
	Wire.send(theByte);
	Wire.endTransmission();
	delay(5);
...
byte readIR()
{
 	...
 	while(bwlFail)
	{
		...
		{
			//thisbyte = Wire.receive(); 
			//}
			thisotherotherbyte = Wire.receive(); 
			thisotherbye = Wire.receive(); 
			thisbyte = Wire.receive(); 
...
//type is as follows: 0: master atmega eeprom, 1: slave atmega eeprom, 2: Ds1307 ram, 3 low eeprom, 4 low eeprom
{
	...
	{	
		Wire.beginTransmission(SLAVE_ADDRESS); 
		Wire.send(itoa(1000+lowstart,10));
		Wire.send("|257|");
		Wire.endTransmission(); 
	}
	return out;
...
{   
	...
	for(i=0; i<=iterations; i++)
	{
		...
		Wire.send(0x51);                                	// Send 0x51 to start a ranging in centimeters
		Wire.endTransmission();
